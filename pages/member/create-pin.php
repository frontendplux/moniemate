...
async function handleVerificationSuccessState() {
    ...
    try {
        ...
        Swal.fire({
            title: "Creating Transaction PIN",
            text: "Please wait...",
        ...
        const response = await fetch("<?= $company_info['server'] ?>", {
            ...
            body: JSON.stringify({
                ...
                action: "/auth/create-transaction-pin",

                pin: primaryPinBuffer
        ...
        disableVirtualKeypadMatrix(false);

        primaryPinBuffer = "";
        confirmationPinBuffer = "";
        isConfirmationPhase = false;

        workflowTitle.textContent = "Create Transaction PIN";

        workflowSubtitle.textContent =
            "Establish a 4-digit security code to authorize token movements, withdrawals, and ledger settlements.";

        stepText.textContent = "PHASE: INITIAL ENTRY";

        progressStep2.classList.remove("bg-success");

        progressStep2.classList.add("bg-light");

        progressStep2.classList.add("border");

        lockShieldIcon.classList.replace(
            "bi-shield-check",
            "bi-shield-lock-fill"
        );
